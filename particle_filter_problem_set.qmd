...
# daysin20years<-9867
# years20<-x[1:daysin20years, ]
# years20L<-tibble(time = 1:daysin20years,
#        as_tibble(years20)) %>% 
#   pivot_longer(cols = -time, names_to = "ensemble", values_to = "x")
# years20L$age<-years20L$time/365


xL %>% filter(age<=20) %>%
  ggplot(aes(x = age, y = x, group = factor(ensemble), color=ensemble)) +
  geom_line(show.legend = F)+
  labs(x = "age (years)", y = "aboveground biomass (gC/m2)") +
  theme_bw()+
  geom_hline(yintercept=6022, linetype=2)
```
