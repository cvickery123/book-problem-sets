...
Soil temperature is in degrees C, so a block distribution ranging from 0 to 100 would be a reasonable prior distribution. I know the soil would not reach water's boiling point, but I do not know the maximum likely temperature of soil to confidently have a prior less than 100 deg C. 
...
```{r}
prior<-tibble(temperature= c(0:100))

ggplot() +
  geom_line(aes(x=prior$temperature), y=1)
