...
# sim_dates.2<-sim_dates[-1]
# for(j in sim_dates.2){
# variance.j<-var_df %>% filter(datetime == j) %>% 
#   group_by(variable, datetime) %>% 
#   summarize(var.sum=sum(variance))
# 
# total_uncertainty<-rbind(total_uncertainty, variance.j)
# }
```
The variance of the combined uncertainty and the sum of the individual variances do not match, as shown in the var.diff column of the var.comparison dataframe. Often, the sum of individual variances is greater than the variance of the combine uncertainty. This means that there is overlap in the variances, which is accounted for in the variance of combined uncertainty but not in the sum of individual variances. Combining all sources of uncertainty produces a smaller variance, so multiple sources of uncertainty produce noise in similar regions. @fig-combo-unc2-parsed depicts this overlap in uncertainties. I would expect, therefore, that adding individual variances would greatly increase variance because the overlap is not accounted for in this approach. 
