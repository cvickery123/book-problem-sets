...
The variance of the combined uncertainty and the sum of the individual variances do not match, as shown in the var.diff column of the var.comparison dataframe. For SOM and Wood, the variance of combined uncertainty is almost exclusively greater than the sum of individual variance for each date (@fig-var-diff). For NEE, the sum of individual variances is greater than the variance of the combine uncertainty. For LAI, the variance of combined uncertainty is greater than the sum of individual variance for the first few weeks of the model horizon. Then, the sum of individual variance becomes almost exclusively greater than the variance of combined uncertainty later in the model horizon.
...
var.comparison %>% filter(variable %in% c("lai", "wood", "som", "nee")) %>%
  ...
  scale_color_stepsn(
    ...
    breaks = 0)
```
When the sum of individual variance is greater than the variance of combined uncertainty, there is overlap in the variances, which is accounted for in the variance of combined uncertainty but not in the sum of individual variances. Combining all sources of uncertainty produces a smaller variance, so multiple sources of uncertainty produce noise in similar regions. @fig-combo-unc2-parsed depicts this overlap in uncertainties. I would expect, therefore, that adding individual variances would greatly increase variance because the overlap is not accounted for in this approach. For LAI, the widening of process uncertainty beyond initial condition uncertainty halfway through the model horizon visually corresponds with the sum of individual variances overcoming variance of combined uncertainty from  @fig-var-diff. It makes sense that NEE has a higher sum of individual variances because NEE is the only variable that has all four sources of uncertainty. 

```{r}
#| warning: FALSE
#| fig-cap: Forecast with all sources of uncertainty, with uncertainties separated by color. 
...
combined_df %>% filter(variable %in% c("lai", "wood", "som", "nee")) %>%
  ...
  scale_fill_manual(values=c("goldenrod2", "gray30", "blue", "red"), name="uncertainty source")+
  geom_line(aes(y = median)) +
  facet_wrap(~variable, scale = "free") +
  theme_bw()
```
